...
- [x] Write failing tests for global inheritance, project include/exclude, run override, stable ordering, conflicts, unknown locators, deltas, and minimal project layer generation.
- [x] Run `uv run pytest tests/test_resolution.py -v`; confirm failures are missing imports/behavior.
- [x] Implement immutable dataclasses/enums and the pure resolver.
- [x] Re-run the focused test and full existing suite.
...
- [x] Write failing tests for Git-root/cwd detection, all locator kinds, TOML round-trip, unknown schema, conflicts, empty config, atomic replacement, and symlink escape rejection.
- [x] Run the focused tests and confirm expected red state.
- [x] Implement strict parsing and centralized realpath containment.
...
- [x] Write failing contract tests using the current official fields: `name`, `description`, `enabled`, `path`, `scope`, `interface`, `dependencies`.
- [x] Write failing filesystem tests for the two standard roots, direct children only, symlink deduplication, malformed metadata, no script execution, and degraded mode.
- [x] Implement safe normalization, frontmatter parsing, portable locator assignment, stable scope grouping, and metadata fingerprints.
- [x] Run focused tests, dependency lock check, and full tests.
...
- [x] Write failing tests for initialize/initialized/skills-list JSONL, request-id matching, timeout, malformed JSON, RPC errors, process cleanup, capability degradation, argv construction, no shell, no permission overrides, and exit-code propagation.
- [x] Implement a short-lived stdio client with dependency-injected process spawning.
- [x] Implement one-shot overrides and preflight comparison behind the adapter; unsupported override capability returns degraded status rather than guessing.
...
- [x] Write failing tests for nested ignore creation, per-run JSON, atomic updates, experiment fields, passthrough unknown skills, path privacy, and write failure semantics.
- [x] Implement project-guarded JSON records with UTC timestamps and UUID run IDs.
- [x] Run focused and full tests.
...
- [x] Write failing pilot tests for collapsed groups, navigation, search, skill/package toggles, mixed-state behavior, discard confirmation, review content, dependency warnings, default action, all three launch choices, and degraded UI.
- [x] Implement focused selector/review screens and stable state transitions.
- [x] Run pilot tests and full tests.
...
- [x] Write failing integration tests with fake clients/apps for launch once, save-and-launch, passthrough, config error, discovery degradation, preflight mismatch, record failure, and missing Codex.
- [x] Implement dependency assembly without moving I/O into Textual handlers.
- [x] Preserve `--version` and `--smoke-test`; add an opt-in diagnostic mode only if required by real E2E.
- [x] Run focused and full tests.
...
- [x] Add failing/skip-verified real-Codex contract coverage for discovery and CLI startup through a PTY-safe smoke path.
- [x] Add MIT license and document normal/degraded behavior, `.skilllab/`, macOS support, validation, and privacy boundaries.
- [x] Run all validation commands, content audit, `git diff --check`, and a spec-to-acceptance self-review.
