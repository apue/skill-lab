...
- Codex CLI 提供原生会话 UI，App Server 提供 `skills/list`；通过单一 adapter 复用，不复制 Codex 配置或 TUI。
- uv 已安装并支持 tool workflow；直接复用。
- 当前 workspace 没有可复用的 Python TUI 应用。

## Extension Points

- Codex runtime adapter：隔离所有 CLI/App Server 版本相关逻辑和能力探测。
...
- New code: discovery normalizer、selector UI、三层 resolver、项目配置和运行记录。
